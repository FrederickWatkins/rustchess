...
fn play_checked_moves(moves: &Vec<ChessMove>) {
    let mut game = game::GameTree::<TransparentBoard>::new(TransparentBoard::starting_board());
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("100 checked moves", |b| {
        b.iter(|| play_checked_moves(&moves))
    });
    c.bench_function("100 psuedo-checked moves", |b| {
        b.iter(|| play_pchecked_moves(&moves))
    });
    c.bench_function("100 unchecked moves", |b| {
        b.iter(|| play_unchecked_moves(&moves))
    });
